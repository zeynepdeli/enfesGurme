...
export function FaqCard({ faq, cardBgImage, textureImage }: Props) {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 z-0 bg-cover bg-center  mix-blend-multiply"
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="absolute right-3 top-3 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-[linear-gradient(135deg,#5a2b16,#8f4f25,#b7773f,#5a2b16)] text-[#f5e7d1] shadow-[0_7px_14px_rgba(71,35,16,0.24),inset_0_1px_1px_rgba(255,255,255,0.35)] transition duration-300 group-hover:rotate-90 sm:h-9 sm:w-9"
        aria-label="Soruyu aç/kapat"
      >
        <ChevronDown
          size={16}
      ...
      {faq.topIcon && (
        <div className="pointer-events-none absolute bottom-4 left-4 z-10 h-8 w-8 opacity-[0.32] mix-blend-multiply">
          <Image src={faq.topIcon} alt="" fill className="object-contain" />
        </div>
      )}
    </article>
  );
