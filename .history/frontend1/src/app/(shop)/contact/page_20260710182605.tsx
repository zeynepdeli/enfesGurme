...
function ContactLayout({
  ...
  return (
    <section
      className={`relative z-10 mx-auto flex min-h-[100svh] w-full max-w-[1720px] flex-col pb-6 ${size.pagePt} ${size.pagePx}`}
    >
...
function PageTitle({ title, size }: { title: string; size: ContactSize }) {
  return (
    <div className="mb-4 text-center md:mb-5 xl:mb-4">
      <h1
        className="font-[family-name:var(--font-merienda)] font-bold leading-none text-[#460d07]"
...
function MapPanel({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[18px] border border-[#9b7047]/55 bg-[#efe3cf] shadow-[0_18px_45px_rgba(80,55,28,0.16)] ${size.panelH}`}
    >
      <div
      ...
      <div className="absolute left-1/2 top-1/2 z-20 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center text-center">
        <div
          ...
          style={{ fontSize: Math.max(20, size.title * 0.4) }}
        >
          {contact.mapTitle}
          <br />
          {contact.mapSubtitle}
...
function InfoPanel({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[18px] border border-[#d8bf8a] bg-cover bg-center shadow-[0_18px_45px_rgba(80,55,28,0.16),inset_0_1px_2px_rgba(255,255,255,0.55)] ${size.panelH} ${size.cardP}`}
      style={{
    ...
    >
      ...
      <div className="relative z-10">
        <p
          ...
          style={{ fontSize: size.infoSmall }}
        >
          {contact.infoSmallTitle}
        </p>

        <h2
          className="mt-1 font-serif font-black leading-none text-[#2c1a0e]"
          style={{ fontSize: size.infoTitle }}
        >
          {contact.infoTitle}
        </h2>
              ...
              <>
                ...
                {contact.addressLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="mt-2 inline-block font-bold text-[#7a3b1e] underline underline-offset-4"
                  >
...
function InfoItem({
  ...
  return (
    <div className="flex items-start gap-3 xl:gap-4">
      <CopperIconCircle className={size.iconClass}>
        <Icon
          strokeWidth={1.8}
...
function SocialLinks({
  ...
  return (
    <div
      className={`mx-auto mt-5 grid w-full max-w-[1000px] gap-3 md:mt-6 ${size.socialGrid}`}
    >
      {socials.map((item) => (
        <SocialCard
...
function SocialCard({
  ...
  return (
    ...
    >
      <div
        className={`relative flex w-full items-center gap-3 rounded-full border border-[#d8bf8a] bg-cover bg-center py-2 pl-3 pr-5 shadow-[0_10px_25px_rgba(80,55,28,0.14)] transition group-hover:-translate-y-1 ${size.socialCardH} ${size.socialCardW}`}
        style={{
          backgroundImage: `url('${contact.cardBgImage || "/cardDuvar.png"}')`,
        }}
      >
        ...
        <CopperIconCircle className={size.socialIconClass}>
          <Icon
            className="relative z-10 text-[#2f1a0e]"
            style={{ width: size.socialIconSize, height: size.socialIconSize }}
          />
        </CopperIconCircle>
        ...
        <div className="relative z-10 min-w-0">
          <h3
            ...
            style={{ fontSize: size.socialTitle }}
          >
            {item.title}
          </h3>

          <p
            className="font-bold uppercase tracking-[0.12em] text-[#3d3020]/70"
            style={{ fontSize: size.socialSubtitle }}
          >
            {item.subtitle}
          </p>

          <span
            className="mt-0.5 inline-flex items-center gap-1 font-black uppercase tracking-[0.18em] text-[#7a3b1e]"
            style={{ fontSize: size.socialSubtitle }}
          >
            İncele <Send size={10} />
          </span>
...
function CopperIconCircle({
  ...
  return (
    <div
      className={`relative z-10 flex shrink-0 items-center justify-center overflow-visible rounded-full bg-[#c2815c] bg-cover bg-center bg-no-repeat shadow-[inset_0_2px_5px_rgba(255,230,200,0.35),inset_0_-4px_7px_rgba(80,35,10,0.28),0_1px_2px_rgba(80,35,10,0.15)] ${className}`}
      style={{
        backgroundImage: `url('${textureImage || "/bkrr.png"}')`,
      }}
    >
      {children}
    </div>
...
function CardBg({ rounded = false }: { rounded?: boolean }) {
  return (
    <>
      ...
      <div
        className={`pointer-events-none absolute inset-[6px] z-[2] border border-[#d6c49a]/70 ${
