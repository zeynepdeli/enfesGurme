...
// Başında + olmayacak. Örn: 905XXXXXXXXX
...
export default function CheckoutPage() {
  ...
  const { data: products, isLoading: productsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<ProductLite[]>(
...
Siparişimi tamamlamak istiyorum.
...
Ödeme ve stok onayı için iletişime geçmek istiyorum.
`.trim();
  };

  const handleCompleteOrder = async () => {
    if (!selectedAddressId || isProcessing || productsLoading) return;
    ...
    try {
      ...
      if (!orderRes.data?.id) {
        throw new Error("Sipariş talebi oluşturulamadı.");
      }
      ...
      sessionStorage.setItem("whatsapp_order_url", whatsappUrl);

      router.push(`/checkout/whatsapp-sent?orderId=${orderId}`);
    } catch (error: any) {
      setErrorMessage(
        ...
        <main className="flex-1 px-5 pb-24 pt-10 sm:px-8 lg:px-12">
          <div className="mx-auto max-w-[1680px]">
            <div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-[#a67c3d]">
              ...
              <span className="text-[#3d3020]/50">Sipariş</span>
            </div>
            ...
            <div className="mb-10 border-b border-[#3d3020]/10 pb-6">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-[#5e4734]/75">
                Siparişiniz ödeme onayı alınmadan kesinleşmez. WhatsApp
                üzerinden ödeme ve stok bilgisi netleştirildikten sonra işleme
                alınır.
              </p>
            </div>
              ...
              <aside>
                <div className="sticky top-36 overflow-hidden rounded-[20px] border border-[#d0bc90] bg-[#efe6cf] p-6 shadow-[0_14px_28px_rgba(120,92,58,0.14),inset_0_1px_2px_rgba(255,255,255,0.55)]">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="mt-4 rounded-xl border border-[#d8bf8a]/70 bg-[#fff5ea]/60 px-4 py-3 text-xs leading-relaxed text-[#5e4734]">
                      Bu aşamada ödeme alınmaz. Siparişi tamamladıktan sonra
                      WhatsApp’a yönlendirilirsiniz.
                    </div>
