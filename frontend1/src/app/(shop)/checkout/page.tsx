...
export default function CheckoutPage() {
  ...
  const handleCompleteOrder = async () => {
    ...
    try {
      ...
      }>("/api/payment/iyzico/init", { orderId });

      if (!paymentRes.data?.checkoutFormContent || !paymentRes.data?.token) {
        throw new Error("Ödeme başlatılamadı");
      }

      sessionStorage.setItem(
        "iyzico_form",
        paymentRes.data.checkoutFormContent,
      );
      sessionStorage.setItem("iyzico_token", paymentRes.data.token);
      sessionStorage.setItem("iyzico_order_id", orderId);
